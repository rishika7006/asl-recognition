...
CROP = 96
GRID = 12
EXPECTED_FEAT_DIM = GRID * GRID * 2  # 288
EXPECTED_PAIRS = NUM_FRAMES - 1  # 15
...
def test_grid_pool() -> None:
    ...
    # Even-indexed positions = u (should be 1.0 everywhere).
    assert np.allclose(pooled[0::2], 1.0)
    # Odd-indexed positions = v (should be 0 everywhere).
...
def test_hand_cropper_random() -> None:
    ...
    try:
        ...
        # On random pixels MediaPipe is unlikely to detect a hand;
        # however some randomness can fool it. Either outcome is OK.
        ...
        # Single-frame variant must also be callable on a fresh frame.
...
def test_augmentation_helpers() -> None:
    ...
    # _augment_clip preserves shape + dtype.
    ...
    # _negate_u_channel flips even-indexed positions.
...
def _make_synthetic_features(
    ...
    def synth(n: int, seed_offset: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i in range(n):
            ...
            # Per-class signature: each class gets a constant offset on
            # a different feature index, plus mild noise.
...
def test_train_end_to_end() -> None:
    ...
    try:
        ...
        # Sanity: well-separated synthetic data should be ~perfectly classified.
