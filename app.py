...
# OpenCV's macOS AVFoundation auth dialog requires the main run loop,
# which Flask worker threads don't have. Skip OpenCV's auth request and
# verify camera access ourselves on the main thread at startup.
...
# Override with ``ASL_CAMERA_INDEX=1`` when macOS Continuity Camera grabs your iPhone.
CAMERA_INDEX = int(os.environ.get("ASL_CAMERA_INDEX", "0"))

# Stride=2 over a ~30 fps camera matches the ~15 fps temporal density used at training.
CAMERA_STRIDE = 2

MIN_CONFIDENCE = 0.45

FB_TICK_SECONDS = 1.0 / 12.0
RAFT_TICK_SECONDS = 1.0 / 6.0
...
class SharedState:
    ...
    def __init__(self, num_frames: int) -> None:
        ...
        # Each entry is (counter, frame). The monotonic counter lets workers
        # detect "new frames since last tick" without comparing arrays.
        self.cropped_buffer: deque[Tuple[int, np.ndarray]] = deque(
            maxlen=num_frames
        )
        self.frame_counter: int = 0

        # For UI drawing only — workers don't read this.
...
class BackendClassifier:
    ...
    def predict(self, features_flat: np.ndarray) -> Tuple[int, float, str]:
        ...
        if self.svm is not None:
            # Decision_function + softmax — the trained SVM has probability=False.
...
def _ema_bbox(
    ...
    if new is None:
        # Keep the previous box so brief detection misses don't make it flicker off.
        return prev
    if prev is None:
        return new
    return tuple(
...
def _draw_overlay(
    ...
    # Translucent backdrop keeps text readable over varied backgrounds.
...
<head>
    ...
    <style>
        ...
        .history h2 { margin: 0 0 12px 0; font-size: 1.1rem; color: #bb86fc; text-align: center; }
        .history-list {
...
<body>
    ...
    function update() {
        fetch("/predictions").then(r => r.json()).then(d => {
            ...
            if (agree) {
                if (fb.label !== lastAgreedLabel) {
                    lastAgreedLabel = fb.label;
                    const list = document.getElementById("history-list");
                    const li = document.createElement("li");

                    const labelSpan = document.createElement("span");
                    labelSpan.innerText = fb.label;
            ...
            } else if (fb.label === "—" && raft.label === "—") {
                // Reset when both backends return to no-prediction so the
                // same sign can be re-logged on a second performance.
                lastAgreedLabel = null;
            }
...
def _verify_camera_access_or_exit() -> None:
    ...
    On macOS, ``cv2.VideoCapture(N)`` from a worker thread fails silently if
    the host app lacks camera access, and Continuity Camera will hijack index
    0 with iPhone if nearby. This probe surfaces both situations.
    """
