...
Pipeline per video, per implementation-plan §5.1:

    1. Read raw .mp4, get total frame count.
    2. Use the action window [start, end] from nslt_100.json to limit
       sampling to the actual sign — this avoids picking up frames
       before/after the gloss.
    3. Sample ``num_frames`` (default 16) evenly-spaced frames inside
       the window.
    4. Run MediaPipe Hands on those frames, take the union bbox,
       expand 30%, square it, clamp to bounds. Crop+resize to 96x96.
    5. For each consecutive pair, compute optical flow with the chosen
       backend (Farnebäck or RAFT).
    6. Pool flow with a 12x12 grid mean → 144 cells x 2 channels = 288
       features per pair → ``(15, 288)`` per clip.
    7. Stack by subset and write npy files plus a StandardScaler fitted
       on the train split.

The landmark concatenation from the previous extractor is **dropped**.
MediaPipe's only role here is producing the bbox.
"""
...
def _sample_indices_in_window(
    ...
    if e <= s:
        # Bad / missing window — use the whole video.
        s, e = 0, last
    if e == s:
        return [s] * num_frames
...
def extract_clip_features(
    ...
    if len(frames) < 2:
        return None
    # Pad with the last frame if any reads failed so we get a consistent
    # T = num_frames input downstream.
...
def run_extraction(
    ...
    # Build the backend + estimator (RAFT loads a model here).
    ...
    try:
        ...
        # ------------------------------------------------------------------
        # Optional: user-recorded clips at data/user_videos/<class>/<clip>.
        # These all go into the TRAIN split (no val/test contamination —
        # 1-2 clips per class would be a useless val signal).
        # ------------------------------------------------------------------
    ...
    # Fit StandardScaler on train (flattened) and persist.
    if X_dict["train"]:
