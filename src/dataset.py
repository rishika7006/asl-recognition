...
The Gaussian-noise "augmentation" in the legacy pipeline was applied
*after* feature extraction, which is meaningless: random noise on a
148-dim pooled-flow vector doesn't produce realistic motion variations.
This Dataset moves augmentation upstream — flips/rotations/translations
are applied to the **raw frames before flow is computed**, so the
classifier sees genuinely different flow distributions per epoch.

Implementation choice (per the task brief):
    Option (b) — operates on raw .mp4 files, doing
    sample → augment → crop → flow on every __getitem__.

Why (b):
    - We don't have to maintain a separate cache of cropped clips on disk.
    - The hand bbox is computed once from the original (un-augmented)
      frames per video and reused, so MediaPipe runs once per video, not
      once per epoch.
    - For val/test we cache the full feature vector after first call,
      so subsequent epochs are fast and deterministic.

Augmentations (training only):
    - Random horizontal flip (50%): u-channel of resulting flow is
      negated to keep the flow consistent with the flipped image.
    - Brightness / contrast jitter (±15%).
    - Random rotation (±5°) + translation (±8 px), applied identically
      to all 16 frames so the motion stays coherent.
    - Temporal jitter: random offset within the action window of ±10%
      of the window length, plus ±10% stride variation.
"""
...
def _augment_clip(
    ...
    # Single rotation+translation matrix reused for the whole clip.
    ...
    for f in frames:
        ...
        # Photometric jitter, clamp back to uint8.
...
class ClipFeatureDataset(Dataset):
    ...
    def __init__(
        ...
        # bbox cache: video_id -> bbox (so MediaPipe runs once per video)
        self._bbox_cache: Dict[str, Optional[Tuple[int, int, int, int]]] = {}
        # eval feature cache: video_id -> (T-1, F) np.ndarray
    ...
    def _features_for_entry(self, entry: Dict[str, Any]) -> np.ndarray:
        ...
        # Eval: single-shot deterministic features, cached.
        ...
        if frames is None:
            # Return zeros so __getitem__ doesn't blow up; in practice
            # _build_video_index already filters missing files. This is
        ...
        # If the augmentation flipped the frames, the cached bbox (in
        # unflipped coords) needs to be flipped horizontally too so it
        # still encloses the hand in the augmented frames.
        ...
        if flip:
            # Convention: negate the u (horizontal) channel so the
            # post-flip flow is consistent with what would have been
            # produced from the un-flipped frames.
