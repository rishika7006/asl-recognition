...
The previous pipeline used MediaPipe landmarks as a feature concatenated
to the optical flow vector. That hurt rather than helped on WLASL —
MediaPipe silently fails on low-resolution YouTube frames, so 126 of the
254 features were systematically zeroed out. Per the implementation plan
(§4.d, §5.1), MediaPipe's role is now reduced to producing a square
bounding box around the signer's hands so we can crop and zoom in
before computing optical flow. That alone is the single biggest fix to
...
Implementation notes:
    The legacy ``mp.solutions.hands`` API was removed in mediapipe>=0.10.
    We use the new ``mediapipe.tasks.vision.HandLandmarker`` API and
    auto-download the model file the first time the cropper is built.
    If the download fails (offline machine, sandbox, etc.) the cropper
    falls back to returning ``None`` from ``bbox_for_clip`` and
    ``bbox_for_frame``; ``crop_clip`` then center-crops, so the rest of
    the pipeline keeps working at lower quality.
"""
...
# MediaPipe is noisy on import; suppress its absl logs to keep CLI clean.
...
# Canonical MediaPipe hand_landmarker model. Cached under ~/.cache/mediapipe.
_MODEL_URL = (
...
class HandCropper:
    ...
    def _detect_landmarks_xy(
        ...
        # MediaPipe expects an mp.Image with SRGB layout.
        try:
    ...
    def _square_bbox_from_points(
        ...
        # Re-square after clamping (shrink to the smaller in-bounds side).
