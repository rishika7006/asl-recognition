...
NUM_PAIRS_TO_SHOW = 4  # number of frame pairs in the figure
CROP_SIZE = 96
...
def _flow_to_color(flow: np.ndarray) -> np.ndarray:
    ...
    # Hue: direction in [0, 180] for cv2 HSV.
    hsv[..., 0] = (ang * 180 / (2 * np.pi)).astype(np.uint8)
    hsv[..., 1] = 255
    # Value: normalized magnitude, capped to a sane range so a single
    # large outlier doesn't black out the rest of the image.
...
def _render(
    ...
    # We need num_pairs pairs → (num_pairs + 1) consecutive-ish frames.
